...
/// <summary>
/// The one home for dzl's JSON conventions: snake_case property names and enums serialized
/// as snake_case strings (e.g. <c>FindingSeverity.Error</c> → <c>"error"</c>,
/// <c>ToolKind.LaunchOnly</c> → <c>"launch_only"</c>). <see cref="JsonStringEnumConverter"/>
/// still <b>reads</b> bare ints, so files written before the converter existed
/// (build cache, proc state, …) deserialize unchanged.
/// </summary>
