...
public sealed class WorkshopService
{
    ...
    public WorkshopOp Download(string id)
    {
        ...
        return SteamCmd.Download(cfg.SteamCmdPath, cfg.SteamLogin, id)
    ...
    public List<string> Downloaded()
    {
        ...
        var dir = Path.Combine(Path.GetDirectoryName(cfg.SteamCmdPath) ?? ".", "steamapps", "workshop", "content", SteamCmd.AppId);
    ...
    {
        var cfg = Cfg;
        return string.IsNullOrWhiteSpace(cfg.SteamCmdPath) ? null : SteamCmd.ContentDir(cfg.SteamCmdPath, id);
    }
}
